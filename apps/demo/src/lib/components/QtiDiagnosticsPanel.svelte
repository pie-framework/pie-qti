...
{#if report}
	<section class="card bg-base-100 shadow-xl" aria-labelledby="qti-diagnostics-title">
		<button
			type="button"
			class="card-body qti-diagnostics-summary"
			aria-expanded={isOpen}
			aria-controls="qti-diagnostics-content"
			onclick={() => (isOpen = !isOpen)}
		>
			<div class="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
				<div>
					<span class="block text-xs uppercase tracking-wide text-base-content/60">Import compatibility</span>
					<span id="qti-diagnostics-title" class="card-title">QTI diagnostics</span>
				</div>
				<span class="badge {statusTone} badge-lg" role="status" aria-live="polite">{statusLabel}</span>
			</div>
		</button>

		{#if isOpen}
			<div id="qti-diagnostics-content" class="card-body gap-4 pt-0">
...
<style>
	.qti-diagnostics-summary {
		cursor: pointer;
		width: 100%;
		text-align: left;
	}

	.qti-diagnostics-summary::before {
		content: '▸';
	...
	.qti-diagnostics-summary[aria-expanded='true']::before {
