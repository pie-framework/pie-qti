...
export interface TypesetActionParams {
	 ...
	 * Optional - if not provided, the action becomes a no-op.
	 */
 ...
 *
 * If typeset is undefined, the action becomes a no-op (returns immediately).
 */
export function typesetAction(node: HTMLElement, params?: TypesetActionParams) {
	// No-op if no typeset function provided
	if (!params?.typeset) {
