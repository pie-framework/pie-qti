import ItemRenderer from '@pie-qti/assessment-player/components/ItemRenderer.svelte';
import type { PlayerSecurityConfig, QTIRole } from '@pie-qti/item-player';
...
export class QtiItemPlayerElement extends BaseSvelteMountElement<Record<string, unknown>> {
	...
	getProps() {
		return {
			...
			onResponseChange: (responseId: string, value: unknown) => {
				this.#responses = { ...this.#responses, [responseId]: value };
				const detail: QtiItemPlayerResponseChangeDetail = {
					responseId,
					value,
					responses: { ...this.#responses },
				};
				this.dispatchEvent(
					new CustomEvent<QtiItemPlayerResponseChangeDetail>('response-change', {
						detail,
						bubbles: true,
						composed: true,
					}),
...
export function defineQtiItemPlayerElement() {
	if (!customElements.get(QTI_ITEM_PLAYER_TAG)) {
		customElements.define(QTI_ITEM_PLAYER_TAG, QtiItemPlayerElement);
	}
}
