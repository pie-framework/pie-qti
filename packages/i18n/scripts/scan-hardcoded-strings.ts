 ...
 *   bun run scripts/scan-hardcoded-strings.ts --path=../default-components
 */
...
async function main() {
	...
	if (!customPath) {
		scanPaths = [
			join(__dirname, '../../item-player/src'),
			join(__dirname, '../../assessment-player/src'),
			join(__dirname, '../../default-components/src'),
		];
	}
