		...
		const ed = new Editor({
			...
			editorProps: {
				attributes: {
					...
					role: 'textbox',
					'aria-multiline': 'true',
					'aria-label': placeholder || 'Extended response editor',
				},
			},
			onUpdate: ({ editor: ed }) => {
...
<div class="richtext-editor" data-readonly={editable ? 'false' : 'true'}>
	...
	<div class="tiptap-editor-container" bind:this={el}></div>
</div>
