...
const { html, trustedTypesPolicyName, interactions, responses, onResponseChange }: Props = $props();
...
const segments = $derived.by(() => {
	...
	const source = htmlToString(html);
	let lastIndex = 0;
	let match;
	...
	while ((match = combinedPattern.exec(source)) !== null) {
		...
		if (match.index > lastIndex) {
			result.push({
				type: 'html',
				content: toTrustedHtml(source.substring(lastIndex, match.index), trustedTypesPolicyName),
			});
		}
	...
	if (lastIndex < source.length) {
		result.push({
			type: 'html',
			content: toTrustedHtml(source.substring(lastIndex), trustedTypesPolicyName),
		});
	}
	...
	if (result.length === 0) {
		result.push({
			type: 'html',
			content: toTrustedHtml(source, trustedTypesPolicyName),
		});
	}
