...
Derive the answer key, partial credit, item weight and choice layout from the QTI source
...
declare a `correctResponse` at all. Such an item converted with an *empty answer key* —
...
- **Only a strictly positive `mappedValue` marks a key correct.** Real Savvas content
  ships `<mapping defaultValue="-0" lowerBound="0" upperBound="0">` with *every*
...
the same assumption. Cardinality is derived *after* the answer key so a mapping-derived
...
degrade gracefully — it misplaces *every* region, which reads as a content defect rather
