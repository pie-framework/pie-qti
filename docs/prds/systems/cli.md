...
- **Security:** The CLI reads files from the local filesystem only. ZIP extraction uses `extractZipToDirSafe` from `@pie-qti/to-pie/ims-cp`, which must prevent path-traversal attacks (entries with `..` components in their paths must be rejected). The CLI does not start any network listeners or make outbound network requests.
