...
The QTI section player is the composition-only render layer for one active item within a QTI section, or one section-like preview slice, above `@pie-qti/item-player` and below `@pie-qti/assessment-player`. It normalizes QTI 2.1 shared passages, QTI 2.2/3.0 stimulus resources, section/testPart rubric blocks, catalogs, package-relative assets, per-item delivery contexts, and active-item navigation into a neutral section render contract. When assessment-player supplies a live `ItemSession`, section-player passes that exact reference to the item element; it does not construct, restore, or synchronize another runtime. The companion QTI assessment toolkit remains a thin QTI-facing coordination layer for role/view and shared-context semantics.
...
`@pie-qti/assessment-player` historically owned both full-assessment orchestration and a duplicate section-adjacent rendering path. That shallow path let rubric, split-pane, feedback, and item rendering drift from the reusable section render layer, including a path that could construct a second live item runtime. The duplicate assessment modules have now been retired; `AssessmentShell.svelte` delegates those concerns to `@pie-qti/section-player`.

The section player extracts the reusable section/page composition concept without importing `@pie-qti/assessment-player` backend contracts. `@pie-qti/assessment-player` remains the full-assessment orchestration layer; it adapts its `SecureAssessment`, `SecureSection`, navigation state, and active live item session into the neutral section contract and delegates active-section rendering. Composer can build the same neutral contract without a session for standalone preview, allowing the item element to own its own definition/session lifecycle.
...
- **Supported attributes:** section and item identifiers, section/testPart role or view constraints,
  navigation mode, submission mode, ordered item references, item source hrefs, section/testPart shared
  rubric blocks, separately presented direct item rubric blocks, QTI 2.1 passage-like auxiliary
  assets, QTI 3 assessment stimulus references, stylesheets, catalogs, and package-relative asset
  hrefs.
...
- **FR-3:** `QtiSectionItemRef` MUST include item identifier, source path or href, title, `itemXml`, optional fallback responses, an optional live `ItemSession`, and optional per-item `ResolvedItemDeliveryContext` from `@pie-qti/ims-cp-core`. It MUST NOT carry serialized session state as a renderer synchronization channel.
- **FR-4:** `QtiSharedContext` MUST represent QTI 2.1 passage-like auxiliary assets, QTI 2.2/3.0
  stimulus resources, section/testPart shared rubric blocks, test feedback, resolved stylesheets,
  catalog sources, package asset diagnostics, and role/view visibility. Direct `assessmentItem`
  rubrics remain on `ItemPresentation.directRubrics` so section composition does not duplicate them.
...
- **FR-10:** `@pie-qti/section-player` MUST compose `@pie-qti/item-player` elements or wrappers. When the active item ref has a session, it MUST set the item element's JavaScript-only `session` property to that same object and omit response snapshot synchronization. It must not duplicate item extraction, response processing, scoring, restoration, or session ownership.
...
- **Performance:** Section normalization should be linear in the number of item refs and shared resources for the active section. Layout and response-only updates must preserve the injected live session and avoid remounting the item player unnecessarily. Package file resolution should be cached by the host or resolver.
...
**Consequences:** Adapter code remains in `@pie-qti/assessment-player` or a QTI toolkit helper. The active `QtiSectionItemRef.session` is the same object returned by `AssessmentPlayer.getCurrentItemSession()`. Section-player treats it as borrowed and never disposes it. `@pie-qti/section-player` can also be embedded by Composer without constructing a full assessment backend, in which case standalone item-element ownership applies.
...
| Item ref contract | `QtiSectionItemRef` | Provide per-item XML, optional fallback responses, and the exact borrowed live session for an assessment-owned active item | Assessment-player injects its current session; Composer omits it |
...
export type QtiSectionItemRef =
  | (QtiSectionItemRefBase & { session: ItemSession; responses?: never })
  | (QtiSectionItemRefBase & { session?: undefined; responses?: Record<string, unknown> });
```

Each item ref carries `itemXml` and can carry per-item `ResolvedItemDeliveryContext`. The active assessment item may also carry the one authoritative live `ItemSession`; inactive refs do not. Section-player passes the session and delivery context through to `@pie-qti/item-player`. `responses` is a standalone/fallback input only and must be omitted when `session` is present. Serialized state belongs to assessment persistence and `AssessmentItemDefinition.openSession({ restore })`, not this composition contract.
...
- Own `SplitPaneResizer.svelte`, `RubricDisplay.svelte`, `ItemRenderer.svelte`, and related accessibility/focus behavior exclusively in `@pie-qti/section-player`; do not retain duplicate assessment-player implementations.
...
- Adapt current navigation state, current item, shared section/testPart rubric blocks, responses,
  section metadata, per-item delivery context, and the exact active `ItemSession` into
  `ResolvedQtiSectionComposition`; direct item rubrics stay on that session's presentation.
...
- **AC-4a:** Given assessment-player owns the active live session, when `toSectionComposition()` and `ItemRenderer` render that item, then the active ref and item element receive the identical object, inactive refs receive no session, and no serialized-state or response-map synchronization creates another live authority.
...
- Current assessment-player implementation: `packages/assessment-player/src/components/AssessmentShell.svelte`, `packages/assessment-player/src/core/AssessmentPlayer.ts`, `packages/assessment-player/src/integration/api-contract.ts`, `packages/assessment-player/src/integration/ReferenceBackendAdapter.ts`.
- Current section rendering implementation: `packages/section-player/src/components/SectionPlayerSplitPane.svelte`, `packages/section-player/src/components/SectionPlayerVertical.svelte`, `packages/section-player/src/components/SplitPaneResizer.svelte`, `packages/section-player/src/components/RubricDisplay.svelte`, `packages/section-player/src/components/ItemRenderer.svelte`, `packages/section-player/src/components/TestFeedback.svelte`.
